    ...
    methods: {
      ...
      loadImage(url) {
        return new Promise((resolve, reject) => {
          ...
          // image.src = img;
          // image.onload = () => resolve(this.getBase64Image(image))
      ...
      drawQRCode(params) {
        this.ctx.save()
        const {
          width = 0, height = 0, left = 0, top = 0, content, background, color
        } = params
      ...
      drawRect(params) {
        this.ctx.save()
        const {
          background = 'white', top = 0, left = 0, width = 0, height = 0, radius = 0, deg = 0
        } = params
